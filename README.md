...
1. При работе с новым проектом необходимо создать репозиторий в [gitlab](https://gitlab.labmedia.su/labmedia). Названием репозитория будет являться имя компании, например, `EL_Raiffeisen_bank`. В каждом проекте необходимо указывать Branches, именем Branches будет являться версийность `v-0.0.1`, при крупном изменении проекта необходимо создавать новую ветку.
...
В файле `globals/Const.js` устфновить флаги в переменных:
...
В файле `scormApi/scormApi.js` 
- Устфновить флаг в переменной `pipwerks.debug.isActive<Boolian>` Для отладки SCORM.
- Устфновить версию SCORM в переменной `pipwerks.SCORM.version<String>` `2004 или 1.2`
...
## Шрифты
### Установка
Для установки в курс необходимо поместить новый шрифт в формате `.ttf` в папку `src/assets/fonts/`. Все необходимые стили для подключения шрифта будут созданы автоматически.
