<template>
  <div>
    ...
    <div v-for="(quest, key) in questionData" :key="quest.qText">
      <div class="container-center" v-if="questionCounter === key">
        <div class="flex">
          <div class="keybord keybord__small">
            ...
            <p v-if="check" class="text-20 mb-10" v-html="getFeedText"></p>
          ...
          <div class="keybord keybord__large">
            ...
            >
              {{ badge }}: {{ questionCounter + 1 }} / {{ questionData.length }}
            </div>
            ...
            ></p>
            <p class="text-20 text-medium mb-25" v-html="getQuestionText"></p>
...
export default {
  ...
  methods: {
    choyceLitter(key) {
      let answerLength = this.questionData[this.questionCounter].word.length;
    ...
    checkAnswer() {
      ...
      let answer = this.questionData[this.questionCounter].word.toUpperCase();
  ...
  computed: {
    ...
    getFeedTitle() {
      if (this.correctAnswer) {
        return this.questionData[this.questionCounter].feed.correct.title;
      }
      if (!this.correctAnswer) {
    ...
    getFeedText() {
      if (this.correctAnswer) {
        return this.questionData[this.questionCounter].feed.correct.text;
      }
      if (!this.correctAnswer) {
        return this.questionData[this.questionCounter].feed.incorrect.text;
      }
    },
